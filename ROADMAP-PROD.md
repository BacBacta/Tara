...
**Dernière mise à jour** : 2026-08-28 — programme terminé, relecture de sécurité faite, photos stockées et affichées, adaptation 4G, onboarding, espace vendeuse **et back-office** refaits, répétition générale du pilote jouée sur PostgreSQL (`JOUR-J.md`).
**État du code** : V1+V2 complets, Next 16.3.3 + React 19, fournisseurs simulés en dev, SQLite en dev / PostgreSQL prêt pour la prod.
250 tests SQLite + 8 tests PostgreSQL, build sans erreur.
...
| 2026-08-28 | **Refonte visuelle « premium »** des parcours publics (accueil, vitrine, fiche, paiement, états), sur demande de MIKE. Le soin, pas le poids : une seule police de titrage (Sora 800, auto-hébergée par next/font, **15 Ko** préchargés), ombres teintées d'indigo, grain SVG de 400 octets en pseudo-élément, boutons avec relief et enfoncement en CSS pur. **Vitrine complète : 28 Ko gzip hors images.** Zéro JavaScript ajouté, formulaires POST natifs intacts (vérifié au curl). | R2 et le budget 3G sont des invariants : le premium devait venir de la typographie, de la lumière et des détails, pas des kilo-octets. |
| 2026-08-28 | La fiche article gagne une **barre d'achat collante** (position:sticky, CSS seul) : WhatsApp, MoMo et la mention R1 ne passent plus jamais sous la ligne de flottaison. | Réponse définitive au point mesuré lors du branchement des photos (bouton à 708 px du haut sur un écran de 640 px). |
| 2026-08-28 | La page d'accueil disait encore « Bio·Shop » — l'ancien nom, oublié au renommage. Corrigée en « tara. ». | Trouvé en relisant la page pour la refonte. |
...
| 2026-08-28 | **Répétition générale du pilote**, jouée d'un bout à l'autre sur un PostgreSQL vide : création de l'admin et de la première vendeuse par les scripts, réglage MoMo, deux articles avec photo, acheteuse arrivant avec un *user agent* TikTok, commande, annonce de paiement, traitement par la vendeuse, avis déposé, encaissement d'abonnement, suspension, export CSV, pré-vol. Elle a trouvé **trois défauts** que les vérifications par lot ne pouvaient pas voir, tous corrigés dans la même PR (voir ci-dessous). Procédure consignée dans **`JOUR-J.md`**. | Chaque lot avait été vérifié séparément ; la journée complète ne l'avait jamais été. Les trois défauts trouvés vivaient précisément *entre* les lots. |
| 2026-08-28 | **Le back-office passe au même système visuel** : cadre partagé `AdminShell` (marque, identité, onglets Boutiques/Pilote, export, déconnexion), tableaux sous une seule définition CSS (`.tbl`), palette du design system. Deux ajouts de fond : un bloc **« À relancer »** en tête (abonnements à échéance sous 7 jours, avec le bouton WhatsApp vers la vendeuse) et **l'activation manuelle d'un abonnement remontée** juste sous les compteurs. | C'était la dernière surface sans le design system — 750 lignes, 98 classes de gris génériques. C'est aussi celle où MIKE vit pendant le pilote : sans agrégateur, encaisser un abonnement à la main est le geste le plus fréquent, et la question du pilote est « combien ont repayé une deuxième fois ». |
| 2026-08-28 | **Préversion déployée sur Vercel** (demande de MIKE) : projet `tara` sous `bactas-projects`, base **Neon** (offre gratuite, Francfort), store **Blob public** pour les photos, fonctions forcées en `fra1` — même région que la base, sinon chaque requête SQL traverse l'Atlantique. Vérifié en direct sur l'URL publique : santé, connexion admin, inscription d'une vendeuse, **envoi d'une photo dans Blob avec ses trois tailles** (49 596 / 40 580 / 22 190 octets), commande, message WhatsApp, image d'aperçu rendue par sharp. | Vercel n'a **pas de shell** : ni migration ni compte administrateur ne peuvent être lancés à la main. La commande de build s'en charge (`db:migrate && create-admin && next build`), et `create-admin.mjs` ne fait rien tant que `ADMIN_EMAIL`/`ADMIN_PASSWORD` ne sont pas définis — il peut donc rester dans le build sans risque. **Ce n'est pas l'ouverture au public** : fournisseurs simulés (les codes OTP s'affichent à l'écran) et pages légales incomplètes. |
| 2026-08-30 | **Le tableau « Navigateurs observés » n'ampute plus le *user agent*** (il le coupait à 90 caractères) et affiche par où la visite est arrivée. | Les marqueurs d'un navigateur intégré — `BytedanceWebview`, `musical_ly` — arrivent **à la fin** de la chaîne, après le préfixe Chrome. L'écran construit pour vérifier l'heuristique du canal effaçait donc exactement sa propre preuve : sur une vraie visite depuis un téléphone, il était impossible de dire si Tara s'était trompée. Découvert en essayant de fermer la question n° 5. |
| 2026-08-28 | **Le numéro de la cliente**, attaché par la vendeuse depuis l'écran Commandes (`setBuyerPhone`, `sendReviewLink`). | En paiement direct — le seul mode du pilote — `orders.buyer_phone` n'était rempli que par le parcours agrégateur. Deux fonctions étaient donc **muettes en production** : le bouton « Écrire à la cliente » n'apparaissait jamais, et le droit d'avis était créé sans que personne ne reçoive le lien. La vendeuse, elle, a le numéro dans WhatsApp dès le premier message. |
| 2026-08-28 | **L'abonnement sans agrégateur** : l'écran *Tara illimité* explique comment payer (portefeuille MoMo de Tara, référence, bouton WhatsApp) au lieu d'ouvrir un paiement qui n'aboutit pas ; `/app/upgrade/init` refuse alors la demande. Trois variables (`TARA_MOMO_NUMBER`, `TARA_MOMO_OPERATOR`, `TARA_WHATSAPP`) et **un contrôle bloquant au pré-vol**. | La vendeuse appuyait sur « Activer l'illimité » et restait indéfiniment sur « Regarde ton téléphone » : aucun code PIN ne pouvait arriver sans agrégateur. Son seul moyen visible de payer était un cul-de-sac — sur la seule recette du produit. |
| 2026-08-28 | **Dates de l'export CSV** ramenées à un format unique (`dateCsv`). | `creee_le` valait `2026-08-28 20:57:31` et `expire_le` `2026-09-27T21:01:50.627Z` dans la même feuille : les abonnements s'écrivent en ISO depuis JavaScript, le reste au format SQL. Sans conséquence dans le code (tout est relu en `Date`), pénible dans un tableur. |
